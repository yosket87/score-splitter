...
describe('振込台帳', () => {
  it('同一操作は保存済み結果を返す', async () => {
    ...
    expect(await replayOperation(mockDb({ input_json: 'input', result_json: JSON.stringify(result) }), 'id', 'input')).toEqual(result)
  })
  it('同じキーの別入力は409', async () => await expect(replayOperation(mockDb({ input_json: 'old' }), 'id', 'new')).rejects.toMatchObject({ status: 409 }))
  it('操作・振込・revisionを単一batchに含める', async () => {
    const db = mockDb()
    await writeOperation(db as D1DatabaseLike, runtime, { operationId: 'id', month: '202609', expectedRevision: 0, kind: 'void', inputJson: '{}', actor: { person: null, authMethod: 'password' }, payment: null, voidPayment: { id: 'payment', reason: '誤操作' } })
  ...
  it('DBで検出した競合を409へ変換', async () => {
    ...
    await expect(writeOperation(db, runtime, { operationId: 'id', month: '202609', expectedRevision: 0, kind: 'void', inputJson: '{}', actor: { person: null, authMethod: 'password' }, payment: null, voidPayment: null })).rejects.toMatchObject({ status: 409 })
  })
})
