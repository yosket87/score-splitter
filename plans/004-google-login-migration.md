...
- [x] 最新mainからブランチを作成し、関連PR・適用済みmigration・依存関係を再確認する。
- [x] Workers上のOIDC検証とADR、環境設定名を確定する（実Googleクライアントの登録・設定は未実施）。
- [x] バックアップの新schema対応と、非空fixtureの復元検証を用意する。
- [x] DBの互換追加と制約拡張を、失敗注入を含むテスト先行で実装する。
- [x] 個人セッション・所属確認・失効を共通境界に実装する。
- [x] OAuth入口と一度きりの移行処理・運営CLIを実装する。
- [x] ログイン/設定/移行案内、モック、全端末ログアウトと復旧運用を完成させる。
- [x] 全家計経路・旧HTTP・振込履歴・移行競合・E2Eを検証する。
- [x] 確認後の旧認証終了処理と全入口の停止判定を実装し、隔離D1・画面・回帰試験で検証する。
