...
describe('振込アダプター', () => {
  ...
  it('通常運用では共有D1関数を呼ぶ', async () => {
    ...
    expect(await getPaymentStatus('202602')).toEqual({ state: 'paid' })
    expect(await recordPayment(input, actor)).toEqual({ paymentId: 'p' })
    expect(mocks.record).toHaveBeenCalledWith(mocks.db, mocks.runtime, input, actor)
    expect(await correctPayment(correction, actor)).toEqual({ voidedPaymentId: 'p' })
    expect(await getPaymentOperation('202602', 'op')).toBeNull()
  ...
  it('HTTPモックにはcookieセッションを渡しactor bodyを送らない', async () => {
    ...
    await correctPayment(correction, actor)
