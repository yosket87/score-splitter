...
describe('AI診断のD1直接アクセス', () => {
  it('空白だけの実行トークンは400で拒否する', async () => {
    await expect(acquireDiagnosisLease({ householdId: 'A' }, '202601', '   ')).rejects.toMatchObject({ status: 400 })
    expect(fetch).not.toHaveBeenCalled()
  })

  it('URLの月と診断結果の月が違う場合は400で保存を拒否する', async () => {
    await expect(saveDiagnosis({ householdId: 'A' }, '202602', {
  ...
  it('DB未設定時は旧APIへフォールバックせず安全な500を返す', async () => {
    ...
    await expect(getDiagnosisContext({ householdId: 'A' }, '202601')).rejects.toMatchObject({
  ...
  it('不正な保存済み診断を安全な500で拒否する', async () => {
    ...
    await expect(getSavedDiagnosis({ householdId: 'A' }, '202601')).rejects.toMatchObject({
      status: 500, message: '内部エラーが発生しました',
    })
  })

  it.each([
    ['対象データの更新', () => saveDiagnosis({ householdId: 'A' }, '202601', {
      runToken: 'run-1', inputHash: 'hash-1', analysisVersion: 'v1',
      diagnosis: diagnosisView, expectedSourceRevision: 99,
    })],
    ['分類対象のラベル変更', () => saveExpenseCategories({ householdId: 'A' }, '202601', 'run-1', [{
      expenseIds: ['expense-1'], category: 'housing', expectedLabel: '変更前の家賃',
    }])],
    ['失効リースの解放', () => releaseDiagnosisLease({ householdId: 'A' }, '202601', 'other-run')],
    ['失効リースの保存', () => saveDiagnosis({ householdId: 'A' }, '202601', {
      runToken: 'other-run', inputHash: 'hash-1', analysisVersion: 'v1',
      diagnosis: diagnosisView, expectedSourceRevision: 0,
    })],
  ] as const)('%sを内部エラーではなく409として返す', async (_, operation) => {
    await acquireDiagnosisLease({ householdId: 'A' }, '202601', 'run-1')
  ...
  it('実行中は409、リース解放後のクールダウンは429を返す', async () => {
    await acquireDiagnosisLease({ householdId: 'A' }, '202601', 'run-1')
    await expect(acquireDiagnosisLease({ householdId: 'A' }, '202601', 'run-2')).rejects.toMatchObject({ status: 409 })
    await expect(releaseDiagnosisLease({ householdId: 'A' }, '202601', 'run-1')).resolves.toBeUndefined()
    await expect(acquireDiagnosisLease({ householdId: 'A' }, '202601', 'run-2')).rejects.toMatchObject({ status: 429 })
    expect(fetch).not.toHaveBeenCalled()
  })

  it('同じD1でリース取得・分類・診断保存を行い、保存結果を再取得できる', async () => {
    expect(await getSavedDiagnosis({ householdId: 'A' }, '202601')).toBeNull()
    await acquireDiagnosisLease({ householdId: 'A' }, '202601', 'run-1')
    const context = await getDiagnosisContext({ householdId: 'A' }, '202601')
    await saveExpenseCategories({ householdId: 'A' }, '202601', 'run-1', [{
      expenseIds: ['expense-1'], category: 'housing', expectedLabel: '家賃',
    }])
    expect((await getDiagnosisContext({ householdId: 'A' }, '202601')).expenses[0].aiCategory).toBe('housing')
    await expect(saveDiagnosis({ householdId: 'A' }, '202601', {
      runToken: 'run-1', inputHash: 'hash-1', analysisVersion: 'v1',
      diagnosis: diagnosisView, expectedSourceRevision: context.sourceRevision,
    })).resolves.toEqual(diagnosisView)
    expect(await getSavedDiagnosis({ householdId: 'A' }, '202601')).toMatchObject({
