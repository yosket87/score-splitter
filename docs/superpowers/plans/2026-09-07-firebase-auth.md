...
```
- [ ] 先に実RS256署名fixtureで成功/改竄/別project/issuer/kid/alg/期限/未来iat/auth_time/provider/tenant/不正UIDをテストし、RED確認。
- [ ] joseで検証。公式x509鍵のcache期限とunknown kid、取得失敗、応答サイズ、timeout、redirect拒否。入力token最大16KiB。
- [ ] accounts:lookupへidTokenをPOSTし、唯一のlocalId一致、disabled!=true、validSinceとauth_timeを照合。APIkeyは公開設定だがURL/レスポンスをログしない。通信先固定、lookup結果はキャッシュしない。
- [ ] fetch/clock注入はテスト用の明示factoryに閉じ、通常入口にmock/任意URL設定を持たせない。
- [ ] Nodeとworkerdで同じ実署名の成功・失敗を確認する。
- [ ] 対象テスト、lint、typecheckと独立レビューを通す。
...
```
- [ ] 0013の非空fixtureを入力とし、0014前後の全既存表/列/保存値、FK、trigger、Googleと旧方式の互換性を先行試験。
- [ ] Firebase identity/申請、floorとfirebase session列を追加。決済表を再構築する際は子も全値コピー/双方向EXCEPT、DDL異常を拒否、rollbackの故障注入。
- [ ] UIDに固定した一度だけの許可、既存Google枠との相互排他、同時消費で1名だけ確定、後段失敗で全rollback。
- [ ] session INSERT時に所属・identity・epoch・floor・有効時刻を再検証。全端末logoutで本人floor/epochだけ更新。
- [ ] 同users.idへ新UIDを復旧。旧identityは履歴として残して失効。期限切れ承認や旧epochでの消費を拒否。
- [ ] CLIは既存Wrangler非公開入力契約を利用し、出力や引数へcode/token/個人情報を出さない。inspect/approve-migration/approve-recoveryを提供。
- [ ] backup schema登録と復元後のfloor/session整合性検査を追加し、独立D1とUnitを通す。
...
- [ ] 設定不足・異origin・不正token・別UID更新の拒否を先にテストする。
- [ ] リクエスト時に非公開server設定を読み、projectId/apiKey/authDomainのみclientへ渡す。Google/Apple enabledフラグは個別。
- [ ] Server Actionでoriginとfreshnessを検証、Task1→Task2順に呼び成功後だけCookieを設定する。ID tokenはレスポンス/ログ/DBへ保存しない。
- [ ] SDKのGoogle/Appleログイン、キャンセル・エラー表示、同UIDへ追加link、現在のsession本人とFirebase currentUser一致の確認、logoutを実装する。
- [ ] 有効D1 sessionの同UID更新のみ古いauth_timeを許可する。失効後はSDK token更新のみでは入れない。
- [ ] 開発mockをproductionで封鎖し、正常・空月・375pxのブラウザ検証とE2Eを実施する。

## Task 4: 統合・実環境・PR

- [ ] 全体Unit/Integration、lint、typecheck、独立D1、OpenNext build、E2Eを実行する。
- [ ] 複数タスクの契約と最終差分をcode-reviewer/security-reviewerで確認し、指摘修正を再検証する。
