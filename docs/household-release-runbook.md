...
| 3 利用切替 | 実装・検証後にSHAを確定 | 世帯対応全経路＋0011。全アクセス停止中に切替 |
| 4 最終制約 | 実装・検証後にSHAを確定 | 0012。対応版だけの稼働と所属整合性が前提 |
...
8. 既存セッション、既存パスキー、パスワード、一覧・明細編集・コピー・AI・振込の既存世帯smokeを実施し、0012完了まで公開入口の停止を維持する。本番へ架空世帯を追加しない。

## 段階4と切り戻し

対応版だけが稼働し、不明所属・必要所属NULL・越境参照が0件であることを確認して0012を適用する。制約と保持値の検査および既存世帯smokeの成功後、記録したVersionだけに100%配分して公開入口とGit自動デプロイを再開する。再開前後のVersion・DB UUID・対象SHAを記録する。
