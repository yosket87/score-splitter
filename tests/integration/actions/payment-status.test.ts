...
describe('振込状況Actions', () => {
  beforeEach(() => { vi.resetAllMocks(); mocks.session.mockResolvedValue({ person: null, authMethod: 'password' }) })
  ...
  it('記録者は共有ログインのセッションから導出し成功した月を再検証', async () => {
    ...
    expect(mocks.record).toHaveBeenCalledWith(input, { person: null, authMethod: 'password' })
  ...
  it('取消と訂正もセッション由来の記録者で実行する', async () => {
    ...
    expect(mocks.correct).toHaveBeenCalledWith(correction, { person: null, authMethod: 'password' })
  })
