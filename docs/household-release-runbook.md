...
8. 既存セッション、既存パスキー、パスワード、一覧・明細編集・コピー・AI・振込の既存世帯smokeを実施し、記録したVersionだけに100%配分して再開する。本番へ架空世帯を追加しない。

## 段階4と切り戻し

対応版だけが稼働し、不明所属・必要所属NULL・越境参照が0件であることを確認して0012を適用する。制約と保持値の検査後にGit自動デプロイを再開する。再開前後のVersion・DB UUID・対象SHAを記録する。
