...
try {
  ...
  const actor = {householdId: '3975b870-bbfa-49fd-ae3d-d273c9f6e107', person:null,authMethod:'password'}
  const month = '202609'
  const status = () => api.getPaymentStatus(db, actor, month)
  const record = async (overrides={}) => {
    ...
    return {input,result:await api.recordPayment(db, runtime, actor, input)}
  }
  assert.equal((await status()).state,'unnecessary')
  await db.prepare('INSERT INTO incomes(household_id,id,month,label,amount,person,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)').bind(actor.householdId,'income',month,'給与',31000,'husband','now','now').run()
  ...
  assert.deepEqual(await api.recordPayment(db, runtime, actor, first.input),first.result)
  await assert.rejects(api.recordPayment(db, runtime, actor, {...first.input,paidOn:'2026-09-04'}),e=>e.status===409)
  assert.deepEqual(await api.getPaymentOperation(db, actor, month, first.input.operationId),first.result)
  assert.equal(await api.getPaymentOperation(db, actor, '202610', first.input.operationId),null)
  // 編集が先なら古い見積りを拒否する。
  ...
  await api.correctPayment(db, runtime, actor, correction)
  assert.equal((await status()).remainingSignedYen,500)
  await assert.rejects(api.correctPayment(db, runtime, actor, {...correction,operationId:crypto.randomUUID(),expectedRevision:(await status()).revision}),e=>e.status===409)
  await assert.rejects(api.correctPayment(db, runtime, actor, {...correction,operationId:crypto.randomUUID(),month:'202610'}),e=>e.status===404)
  ...
  for (const [table,flag] of [['expenses','is_carryover'],['carryovers','is_cleared']]) {
    ...
    await db.prepare(`INSERT INTO ${table}(household_id,id,month,label,amount,person,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)`).bind(actor.householdId,table,month,table,-100,'wife','now','now').run()
    ...
    assert.equal((await status()).revision,++rev)
    const moved = (await api.getPaymentStatus(db, actor, '202610')).revision
    await db.prepare(`DELETE FROM ${table} WHERE id=?`).bind(table).run()
    assert.equal((await api.getPaymentStatus(db, actor, '202610')).revision,moved+1)
  }
  ...
  await assert.rejects(api.correctPayment(db, runtime, actor, {month,operationId,expectedRevision:before.revision,paymentId:active.id,reason:'rollback検証',replacement:{signedYen:1,paidOn:'2026-09-05'}}),e=>e.status===500)
  assert.equal(await api.getPaymentOperation(db, actor, month, operationId),null)
  ...
  const getMonthStatus = targetMonth => api.getPaymentStatus(db, actor, targetMonth)
  const recordMonth = async targetMonth => {
    const current = await getMonthStatus(targetMonth)
    return api.recordPayment(db, runtime, actor, {
      month: targetMonth,
  ...
  await api.createRecord(db, runtime, actor, 'income', {
  ...
  for (const type of ['income', 'expense', 'carryover']) {
    ...
    const created = await api.createRecord(db, runtime, actor, type, input)
    assert.equal((await getMonthStatus(crudMonth)).revision, ++revision)
    await api.updateRecord(db, runtime, actor, type, created.id, {
      ...input, label: `更新-${type}`, amount: type === 'income' ? 200 : -200,
    })
    assert.equal((await getMonthStatus(crudMonth)).revision, ++revision)
    if (type !== 'income') {
      const flag = type === 'expense' ? { isCarryover: true } : { isCleared: true }
      await api.patchRecordFlag(db, runtime, actor, type, created.id, flag)
      assert.equal((await getMonthStatus(crudMonth)).revision, ++revision)
    }
    await api.deleteRecord(db, actor, type, created.id)
    assert.equal((await getMonthStatus(crudMonth)).revision, ++revision)
  ...
  await api.createRecord(db, runtime, actor, 'expense', {
    month: sourceMonth, label: 'コピー支出', amount: -200, person: 'wife',
  })
  await api.createRecord(db, runtime, actor, 'carryover', {
  ...
  const preview = await api.getCopyMonthPreview(db, actor, sourceMonth, targetMonth)
  const copyInput = {
    sourceMonth,
    targetMonth,
  ...
  const added = await api.copyMonthData(db, runtime, actor, { ...copyInput, mode: 'add' })
  ...
  const skipped = await api.copyMonthData(db, runtime, actor, { ...copyInput, mode: 'skip' })
  ...
  const replaced = await api.copyMonthData(db, runtime, actor, { ...copyInput, mode: 'replace' })
  ...
  await api.copyMonthData(db, runtime, actor, { ...copyInput, targetMonth: emptyCopyMonth, mode: 'add' })
  ...
  const interleavedIncome = await api.createRecord(db, runtime, actor, 'income', {
  ...
  const interleavedDb = {
    ...
    batch: async statements => {
      ...
      if (!editInserted) {
        editInserted = true
        await api.updateRecord(db, runtime, actor, 'income', interleavedIncome.id, {
  ...
  await assert.rejects(api.recordPayment(interleavedDb, runtime, actor, {
  ...
  assert.equal(await api.getPaymentOperation(db, actor, interleavedMonth, interleavedOperationId), null)
  ...
  const duplicates = await Promise.all([
    api.recordPayment(db, runtime, actor, duplicateInput),
    api.recordPayment(db, runtime, actor, duplicateInput),
  ])
